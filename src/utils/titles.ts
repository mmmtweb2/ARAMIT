...
// ודא שהמילה "export" קיימת לפני הפונקציה
export const getTitleForScore = (score: number): string | null => {
    let highestTitle: string | null = null;
    for (const title of titles) {
        if (score >= title.score) {
            highestTitle = title.name;
        } else {
            // מכיוון שהמערך מסודר, ברגע שהגענו לניקוד גבוה מהנוכחי, אפשר לעצור
            break;
        }
    }
    return highestTitle;
